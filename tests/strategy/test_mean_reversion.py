...
class TestSignalStrength:
    ...
    def test_strength_scales_with_deviation(self):
        """Larger deviations should produce stronger signals (up to cap)."""
        s = MeanReversionStrategy(lookback=5, threshold=1.0)
        closes = [100.0] * 5 + [95.0]  # mild deviation
        signals_mild = _feed_series(s, "SPY", closes)

        s2 = MeanReversionStrategy(lookback=5, threshold=1.0)
        closes2 = [100.0] * 5 + [70.0]  # extreme deviation
        signals_extreme = _feed_series(s2, "SPY", closes2)

        if signals_mild and signals_extreme:
            assert signals_extreme[0].strength >= signals_mild[0].strength
